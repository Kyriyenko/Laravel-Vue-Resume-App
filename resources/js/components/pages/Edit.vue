...
export default {
    ...
    methods: {
        deleteNetwork(id){
            profileService.deleteNetworkData({id:id}).then(response => {
        ...
        addNetworks(){
            ...
            profileService.createNetwork({name:this.networkName,link:this.networkLink})
                .then(response => {
        ...
        updateUserInfo(){
            profileService.edit(this.post).then(response => {
                if(response.data.status===true){
